...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = 'd916399f-e174-418e-ab7d-b44866d411ba'; // ID de NelysNails
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'NelysNails';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '53643129';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'nelysnails2026@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a NelysNails!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'nelysnails';
};
...
console.log('✅ config-negocio.js listo para NelysNails');
